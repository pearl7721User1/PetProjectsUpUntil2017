...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func removeAll() {
        ...
        if let countries = try? persistentContainer.viewContext.fetch(fetchRequest) {
    ...
    func importIfPossible() {
        
        
        let fetchRequest: NSFetchRequest<Country> = Country.fetchRequest()
        let count = try? persistentContainer.viewContext.count(for: fetchRequest)
        
        guard let countryCount = count,
            countryCount == 0 else {
                return
        }
        ...
        if let array = NSArray(contentsOf: URL(fileURLWithPath: plistPath!, isDirectory: false)) as? Array<[String: String]> {
            ...
            for dict in array {
                ...
                let countryManagedObject = Country(context: persistentContainer.viewContext)
                
                countryManagedObject.name = dict["name"]
                countryManagedObject.capital = dict["capital"]
                countryManagedObject.continent = dict["continent"]
                countryManagedObject.population = dict["population"]
