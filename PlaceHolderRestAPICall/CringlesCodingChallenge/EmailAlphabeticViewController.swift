//
//  EmailAlphabeticViewController.swift
...
class EmailAlphabeticViewController: CommentsCommonViewController {

    override func viewDidLoad() {
        super.viewDidLoad()
        
        // if Comments are already set in the CommentsClient instance before this view controller is created, fetch Comments from it manually
    ...
    override func prepare(ForCommentList list: [Comment]) {
        ...
        // sort it as email alphabetical
        self.commentList = list.sorted(by: { (leftValue, rightValue) -> Bool in
            return leftValue.email < rightValue.email
        })
