...
class NotReadyForDownloadView: UIView {
    ...
    convenience init() {
        ...
        bezierPath.addArc(withCenter: CGPoint(x: rect.midX, y: rect.midY), radius: rect.width/2, startAngle: CGFloat(M_PI_2)*3, endAngle: CGFloat(M_PI_2)*6.3, clockwise: true)
        bezierPath.stroke()
        ...
        move.toValue = M_PI_2*4//bar.position.y - 35.0
        move.duration = 1.0
    ...
    class func image() -> UIImage {
        ...
        let bezierPath = UIBezierPath()
        bezierPath.addArc(withCenter: CGPoint(x: boundCircleRect.midX, y: boundCircleRect.midY), radius: boundCircleRect.width/2, startAngle: CGFloat(M_PI_2)*3, endAngle: CGFloat(M_PI_2)*6.3, clockwise: true)
        bezierPath.stroke()
